...
private extension BodyFatEntryModel {
    convenience init(from entry: BodyFatEntry) {
        self.init(id: entry.id, date: entry.date, bodyFatFraction: entry.bodyFatFraction)
    }
}

private extension BodyFatEntry {
    init(from model: BodyFatEntryModel) {
        self.init(id: model.id, date: model.date, bodyFatFraction: model.bodyFatFraction)
    }
}

// MARK: - BodyFatStore

/// Local-only store for body-fat history.
///
/// Backend: SwiftData (`BodyFatEntryModel` via `BulkAISchemaV1`). Migrated
/// from the legacy `"bodyFatEntries"` UserDefaults blob on first launch by
/// `SwiftDataMigration.runIfNeeded(into:)`.
///
/// Mirrors WeightStore but trimmer:
...
final class BodyFatStore {
    ...
    func addEntry(_ entry: BodyFatEntry) {
        let model = BodyFatEntryModel(from: entry)
    ...
    func deleteEntry(_ entry: BodyFatEntry) {
        let id = entry.id
        let descriptor = FetchDescriptor<BodyFatEntryModel>(predicate: #Predicate { $0.id == id })
    ...
    func replaceAllEntries(_ newEntries: [BodyFatEntry]) {
        let existing = (try? context.fetch(FetchDescriptor<BodyFatEntryModel>())) ?? []
        existing.forEach { context.delete($0) }
        newEntries.forEach { context.insert(BodyFatEntryModel(from: $0)) }
        try? context.save()
        rebuild()
    }
    ...
    func importExternalEntries(_ external: [BodyFatEntry]) {
        ...
        external.forEach { context.insert(BodyFatEntryModel(from: $0)) }
        try? context.save()
        rebuild()
        syncProfileBodyFatToLatest()
    }
    ...
    private func rebuild() {
        let models = (try? context.fetch(FetchDescriptor<BodyFatEntryModel>())) ?? []
        entries = models.map { BodyFatEntry(from: $0) }
    }
